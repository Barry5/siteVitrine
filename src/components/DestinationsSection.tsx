...
export const DestinationsSection: React.FC = () => {
  ...
  return (
    <section id="destinations" className="py-20 bg-[#FAF9F6] border-b border-slate-200 scroll-mt-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center space-y-3 mb-16">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-50 text-[#C8102E] font-bold text-xs uppercase tracking-wider border border-red-200">
            ...
            <span>Réseau & Lignes Internationales</span>
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
            Des connexions directes vers vos métropoles clés
          </h2>
              ...
              return (
                <button
                  ...
                  className={`w-full p-4 rounded-2xl text-left border transition-all flex items-center justify-between cursor-pointer ${
                    isSelected
                      ? 'bg-white border-[#C8102E] shadow-md ring-2 ring-red-100'
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-slate-600 font-medium">
                        {dest.country} • Aérien {dest.estimatedAirDays}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {dest.nextScheduledFlight && (
                      <span className="hidden sm:inline-block px-2 py-0.5 rounded text-[10px] font-extrabold uppercase bg-red-50 text-[#C8102E] border border-red-200">
                        {dest.nextScheduledFlight}
                      </span>
                    )}
                    <ChevronRight
                      className={`w-4 h-4 transition-transform ${
                        isSelected ? 'text-[#C8102E] translate-x-1' : 'text-slate-600'
                      }`}
                    />
                  </div>
                </button>
            ...
            {selectedDest && (
              <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pb-5 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-xs text-slate-600 font-medium">
                        Ligne régulière Conakry (CKY) ➔ {selectedDest.name}
                      </span>
                    </div>
                  </div>

                  {selectedDest.nextScheduledFlight && (
                    <div className="bg-gradient-to-r from-[#991B1B] to-[#C8102E] text-white px-3.5 py-1.5 rounded-xl text-xs font-bold shadow-sm">
                      <span className="block text-[10px] text-red-200 uppercase font-semibold">Prochain vol</span>
                      {selectedDest.nextScheduledFlight}
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                    ...
                    <p className="text-[11px] text-slate-600">
                      Prise en charge prioritaire pour denrées fraîches, documents & colis urgents.
                    </p>
                  </div>
                  ...
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-1">
                    <div className="flex items-center gap-2 text-xs font-bold text-slate-700">
                      ...
                      <span>Fret Maritime Groupage</span>
                    </div>
                    <div className="text-xl font-extrabold text-slate-900 font-display">
                      {selectedDest.estimatedSeaDays}
                    </div>
                    <p className="text-[11px] text-slate-600">
                      Recommandé pour les fûts, gros cartons, mobilier et effets volumineux.
                    </p>
                  </div>
                </div>
                ...
                <div className="p-5 rounded-2xl bg-slate-900 text-white space-y-3">
                  <div className="flex items-center justify-between text-xs text-amber-300 font-bold uppercase tracking-wider">
                    <span className="flex items-center gap-1.5">
                      ...
                      Point de retrait & contact local
                    </span>
                    <span className="text-[11px] bg-slate-800 text-slate-300 px-2 py-0.5 rounded">
                      Équipe Thiaguil sur place
                    </span>
                  </div>

                  <div className="space-y-1">
                    <div className="font-extrabold text-base text-white">
                      {selectedDest.localContactName || `Bureau Thiaguil ${selectedDest.name}`}
                    </div>
                    <p className="text-xs text-slate-300 leading-relaxed">
                      {selectedDest.localAddress || 'Adresse communiquée lors de la confirmation d\'envoi'}
                    </p>
                  </div>
                  ...
                  {selectedDest.localPhone && (
                    <div className="pt-2 border-t border-slate-800 flex items-center justify-between flex-wrap gap-2 text-xs">
                      <span className="text-slate-300">Ligne directe sur place :</span>
                      <a
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-2">
                  ...
                  >
                    <span>Simuler le prix vers {selectedDest.name}</span>
                  </a>
                  ...
                  <a
                    ...
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-[#C8102E] hover:bg-[#A60D25] text-white font-bold text-xs transition-colors shadow-sm"
                  >
                    <span>Planifier un envoi</span>
